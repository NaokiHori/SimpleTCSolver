 ...
 */
#define GTSV(type) \
  static int gtsv_##type(const int n, const double * restrict l, const double * restrict c, const double * restrict u, double * restrict v, type * restrict q){ \
...
#define TDM_SOLVE(type) \
  static int tdm_solve_##type(const int n, const int nrhs, const bool is_periodic, const double * restrict l, const double * restrict c, const double * restrict u, double * restrict v, type * restrict q, double * restrict q1){ \
    if(is_periodic){ \
 ...
 * @return                 : error code
 */
static int construct(const int size, const int nrhs, const bool is_periodic, const bool is_complex, tdm_info_t **info){
  // sanitise input
  if(size <= 0){
  ...
  *info = common_calloc(1, sizeof(tdm_info_t));
  // sizes
  (*info)->size = size;
  (*info)->nrhs = nrhs;
  // flags
  ...
  // buffers
  (*info)->l = common_calloc((size_t)size, sizeof(double));
  (*info)->c = common_calloc((size_t)size, sizeof(double));
  (*info)->u = common_calloc((size_t)size, sizeof(double));
  (*info)->v = common_calloc((size_t)size, sizeof(double));
  if(is_periodic && /* to avoid zero-size allocation */ 1 < size){
    (*info)->q1 = common_calloc((size_t)(size-1), sizeof(double));
  }else{
    (*info)->q1 = NULL;
  }
  return 0;
 ...
 * @return          : error code
 */
static int get_l(const tdm_info_t * restrict info, double * restrict *l){
  if(NULL == info){
 ...
 * @return          : error code
 */
static int get_c(const tdm_info_t * restrict info, double * restrict *c){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
 ...
 * @return          : error code
 */
static int get_u(const tdm_info_t * restrict info, double * restrict *u){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
...
static int get_size(const tdm_info_t * restrict info, int * restrict size){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
...
static int get_nrhs(const tdm_info_t * restrict info, int * restrict nrhs){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
 ...
 * @return             : error code
 */
static int solve(const tdm_info_t * restrict info, void * restrict data){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
  ...
  if(is_complex){
    fftw_complex * restrict q = (fftw_complex *)data;
    tdm_solve_fftw_complex(size, nrhs, is_periodic, l, c, u, v, q, q1);
  }else{
    double * restrict q = (double *)data;
    tdm_solve_double(size, nrhs, is_periodic, l, c, u, v, q, q1);
  }
  return 0;
}
 ...
 * @return         : error code
 */
static int destruct(tdm_info_t * restrict info){
  if(NULL == info){
    printf("ERROR(%s): info is NULL\n", __func__);
    return 1;
  }
