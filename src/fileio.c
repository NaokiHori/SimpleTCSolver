...
static char *generate_error_message(const char fname[], const int line, const char message[]){
  /* format is ("%s:%d, %s", filename, line, message) */
  size_t nchars[3] = {0};
  nchars[0] = strlen(fname);
  nchars[1] = (size_t)common_get_ndigits(line);
  nchars[2] = strlen(message);
  // number of all characters + separators (':' and ", ") and NUL, 4 additional characters in total
  const size_t nchars_total = nchars[0] + nchars[1] + nchars[2] + 4;
  /* allocate and copy contents */
  char *retval = common_calloc(nchars_total, sizeof(char));
  snprintf(retval, nchars_total, "%s:%d, %s", fname, line, message);
  return retval;
}
...
static char *generate_npy_filename(const char dirname[], const char dsetname[]){
  ...
  const size_t nchars_total = nchars[0] + nchars[1] + 6;
  char *fname = common_calloc(nchars_total, sizeof(char));
  snprintf(fname, nchars_total, "%s/%s.npy", dirname, dsetname);
  return fname;
}
...
char *fileio_concatenate(const char prefix[], const int step){
  // +1: NUL
  const size_t nchars_total = strlen(prefix) + NDIGITS_STEP + 1;
  char *dirname = common_calloc(nchars_total, sizeof(char));
  snprintf(dirname, nchars_total, "%s%0*d", prefix, NDIGITS_STEP, step);
  return dirname;
}
 ...
 * @param[in,out] data             : pointer to the data, (in) when write, (out) when read
 */
 ...
 * @param[in,out] data              : pointer to the data to be loaded or written
 */
static int kernel_parallel(const bool is_read, const char dirname[], const char dsetname[], const int ndims, const int *array_of_sizes, const int *array_of_subsizes, const int *array_of_starts, double *data){
  ...
  if(myrank == 0){
    // values expected
    size_t *shape = common_calloc((size_t)ndims, sizeof(size_t));
    for(int dim = 0; dim < ndims; dim++){
      shape[dim] = (size_t)array_of_sizes[dim];
    }
    const char dtype[] = {NPYIO_DOUBLE};
    const bool is_fortran_order = false;
    header_size = is_read
      ? fileio_r_npy_header(fname, (size_t)ndims, shape, dtype, is_fortran_order)
      : fileio_w_npy_header(fname, (size_t)ndims, shape, dtype, is_fortran_order);
    common_free(shape);
  }
  ...
  // load / dump
  {
    ...
    const MPI_Offset disp = (MPI_Offset)header_size;
