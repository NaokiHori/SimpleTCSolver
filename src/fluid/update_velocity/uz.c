...
static void solve_linear_systems_in_x(const domain_t * restrict domain, const double prefactor){
  ...
  for(int i = 0; i < isize; i++){
    // N.B. LAPUZX(1) <-> i = 0
    tdm_l[i] =    - prefactor * LAPUZX(i+1).l;
    tdm_c[i] = 1. - prefactor * LAPUZX(i+1).c;
    tdm_u[i] =    - prefactor * LAPUZX(i+1).u;
  }
  tdm_solve_double(
...
static void solve_linear_systems_in_y(const domain_t * restrict domain, const double prefactor){
  ...
  for(int i = ioffs; i < isize + ioffs; i++){
    ...
    for(int j = 0; j < jsize; j++){
      // N.B. LAPUZY(1) <-> i = 0
      tdm_l[j] =    - prefactor * LAPUZY(i+1).l;
      tdm_c[j] = 1. - prefactor * LAPUZY(i+1).c;
      tdm_u[j] =    - prefactor * LAPUZY(i+1).u;
    }
    tdm_solve_double(
...
static void solve_linear_systems_in_z(const domain_t * restrict domain, const double prefactor){
  ...
  for(int k = 0; k < ksize; k++){
    tdm_l[k] =    - prefactor * lapuzz.l;
    tdm_c[k] = 1. - prefactor * lapuzz.c;
    tdm_u[k] =    - prefactor * lapuzz.u;
  }
  tdm_solve_double(
