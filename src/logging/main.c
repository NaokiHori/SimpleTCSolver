 ...
 * @return           : error code
 */
int logging(const domain_t *domain, const int step, const double time, const double dt, const double wtime, const fluid_t *fluid){
  if(!is_scheduled){
    ...
    check_energy    (DIRNAME_LOG "/energy.dat",     domain, time, fluid);
    check_nusselt   (DIRNAME_LOG "/nusselt.dat",    domain, time, fluid);
